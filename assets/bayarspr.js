...
$(".btn_pilih_spr").on("click", function () {
	// alert(baseUrl);
	var namaspr = $("#namaSopir").val();

	var datatosend = {};
	datatosend.namaspr = namaspr;
	...
	$.post(
		baseUrl + "/adm/CpembSpr/get_angkutan",
		datatosend,
		function (response) {
			// alert(response);
...
$(".dsp_4 ").on("click", ".btn_submit_angkutan", function () {
	...
	if (confirm("Data Sudah Benar?")) {
		$.post(
			baseUrl + "/adm/CpembSpr/prosesByr",
			datainsert,
			function (response) {
				...
				if (resp.pesan == "OK") {
					...
					// $(".dsp_4").html(refreshDsp_4());
					var url = baseUrl + "/adm/CpembSpr/notaPrint/" + resp.nota;
					bukawinbaru(url);
					location.reload();
				}
				// $(".t_body").html(response);
...
function refreshDsp_4() {
	...
	var total_harga = 0;
	// var datacust = "";
	var total_biaya_angkut = 0;
	// alert(list_item[0].namabar);
	// return;
	list_item.forEach(function (item, index) {
		total_biaya_angkut =
			parseFloat(item.jum_kubikasi) * parseFloat(item.upah_ang_per_kubik);
		textList += `
					<div class="row ">
						<div class="col">
								...
								<div class="row list_angkutan" data-row = "${index}">
									...
									<div class="col ">
										...
										${
										 ...
										 <br>
										<span class="small">
											${item.namaspr + "|" + item.namapic + "|" + item.namamat}  @
									...
									<div class="col-3 font-weight-bold text-right">
										${formatAngka(total_biaya_angkut)}
									</div>
								</div>
		...
		total_harga += total_biaya_angkut;
	});
