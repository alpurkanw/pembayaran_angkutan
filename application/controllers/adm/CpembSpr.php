...
class CpembSpr extends CI_Controller
{
    ...
    public function index()
    {
        ...
        // $sql = " SELECT * from trx_angkutan
        // where st_byr_spr = 0  and jum_kubikasi <> 0";
        // $data["angks"] =  $this->db->query($sql)->result();


        $this->load->view('adm/VpembSpr', $data);
    }
    ...
    public function get_angkutan()
    {
        ...
        $sql = "SELECT * from trx_angkutan where st_byr_spr = 0 and namaspr = '$namaspr'  and (jum_kubikasi <> 0 or upah_ang_per_kubik <> 0) ";
        $angs  = $this->db->query($sql)->result();

        // $angs = $this->db->get_where('trx_angkutan', ["namaspr" => $namaspr])->result();

        // <th>kode <br>Tanggal Angkut</th>
        // <th>Sopir </Br> PIC <br> Tujuan <br> Material</th>
        // <th>Kubikasi <br> Angkut Per Kubik <br>
        //     Material Per Kubik
        // </th>
        // <th>Total Angkut</th>
        ...
        if (count($angs) > 0) {
            ...
            >
                ...
                <td> 
                    <strong>' . number_format($ank->jum_kubikasi * $ank->upah_ang_per_kubik) . ' </strong>
                </td>
                <td> ' . number_format($ank->jum_kubikasi * $ank->hargaperkubik) . '</td>
            </tr> ';
            ...
            <tr>
                <td colspan="4">
                Data Angkutan yang siap dibayarkan dengan nama sopir : <strong>' . $namaspr . ' </strong>   Belum ada
                </td>
    ...
    public function prosesByr()
    {
        ...
        $this->db->insert('trx_pemb_spr', $data);
    ...
    public function notaPrint($id)
    {
        if ($id !== "") {
            ...
            $this->db->where('nota_byr_spr', $id);
            $this->db->order_by('id', 'ASC');
            $data["trxangks"] = $this->db->get()->result();

            $this->db->select('*');
            $this->db->from('trx_pemb_spr');
            $this->db->where('id', $id);
            $this->db->order_by('id', 'ASC');
            $data["trxByrs"] = $this->db->get()->result();


            $this->load->view('adm/VnotaPrint', $data);
        }
    }
